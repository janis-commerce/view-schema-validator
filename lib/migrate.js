...
function convertFile(inputFile, outputFile, ymljs, currentFileRelDir) {
	...
	const isPartial = inputFile.includes('.partial.') || inputFile.split(path.sep).includes('sections');
	const wrap = src => (isPartial ? src : `defineView(${src})`);

	let jsContent = '\'use strict\';\n\n';

	if(!isPartial)
		jsContent += 'const { defineView } = require(\'@janiscommerce/view-schema-validator\');\n';
	...
	if(uniqueRefs.length > 0) {
		...
		jsContent += 'module.exports = ' + wrap(source) + ';\n';
	} else {
		if(!isPartial)
			jsContent += '\n';
		jsContent += 'module.exports = ' + wrap(toJsSource(parsed)) + ';\n';
	}
