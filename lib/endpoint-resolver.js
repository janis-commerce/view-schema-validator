...
class EndpointResolver {
	 ...
	 */
	static getEndpointResolved(currentEndpoint, endpointResolved) {
	...
	callFetcher(service, namespace, method) {
		...
		objSet(this.requestCache, `${service}.${namespace}.${method}`, promise);
	...
	async getEndpoint({ service, namespace, method }) {
		...
		const promise = objGet(this.requestCache, `${service}.${namespace}.${method}`);
	...
	addResolveDataToEndpoints(endpointsToResolve, endpointsResolved) {
		endpointsToResolve.forEach(currentEndpoint => {
			const endpoint = endpointsResolved.find(endpointResolved => (
				EndpointResolver.getEndpointResolved(currentEndpoint, endpointResolved)
			));

			const { httpMethod, url } = endpoint;
			currentEndpoint.httpMethod = httpMethod.toLowerCase();
