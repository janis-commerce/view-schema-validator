...
class Builder {
	...
	async processFile(filePath, isInitialPathFile = false) {
		if(this.regexExtensions.test(filePath)) {
			...
			const isYaml = /\.yml|\.yaml/g.test(filePath);

			const data = isYaml ? ymljs.parse(file.toString()) : JSON.parse(file.toString());

			const dataBuilded = Validator.execute(data, this.build, filePath);

			return dataBuilded && this.processOutput(dataBuilded, filePath, isInitialPathFile);
		}
