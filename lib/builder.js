...
class Builder {
	...
	async processFile(filePath, isInitialPathFile = false) {
		if(this.regexExtensions.test(filePath)) {
			...
			const fileParsed = isYaml ? ymljs.parse(file.toString()) : JSON.parse(file.toString());

			const data = await refParser.dereference(fileParsed);
	...
	async processInput(currentPath) {
		...
		dir.forEach(fileDir => {
			...
			if(dirFilePath.endsWith('.partial.yml'))
				return;
