...
class Builder {
	...
	/**
	 * Load a view definition file (JS or JSON)
	 * @param {string} filePath
	 * @returns {object} The parsed view definition
	 */
	loadFile(filePath) {
		...
		if(isJs) {
			...
			// Clear require cache for watch mode support
			delete require.cache[require.resolve(absolutePath)]; // eslint-disable-line global-require, import/no-dynamic-require
			return require(absolutePath); // eslint-disable-line global-require, import/no-dynamic-require
		}
	...
	/**
	 * Check file type, parse and validate or build with Validator.execute
	...
	async processInput(currentPath) {
		...
		dir.forEach(fileDir => {
			...
			if(dirFilePath.endsWith('.partial.js') || dirFilePath.endsWith('.partial.json'))
				return;
